    ...
    public partial class NurseLogin : Form
    {
        ...
        private void btn_login_Click(object sender, EventArgs e)
        {
            ...
            if (v == 0)
            {
                ...
                sqlCommand.CommandText = $"SELECT COUNT(1) FROM NURSE " +
                    $"WHERE NUR_ID=@NUR_ID " +
                    $"AND NUR_PASS=HASHBYTES('MD5',@Password);";//指定SQL命令的命令文本，命令包含参数
                #region 账号参数
                SqlParameter sqlParameter = new SqlParameter();
                sqlParameter.ParameterName = "NUR_ID";
                ...
                if (rowCount == 1)
                {
                    ...
                    NurseStation nurseStation = new NurseStation(this.txt_id.Text.Trim());
                    this.Hide();
                    nurseStation.ShowDialog();
                    this.Dispose();
                    this.Close();
                }
                else
